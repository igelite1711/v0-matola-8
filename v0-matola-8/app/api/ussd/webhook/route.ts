...
async function getSession(sessionId: string, phoneNumber: string): Promise<USSDSession | null> {
  ...
  // Enforce session safety
  await invariantEngine.execute(
    ["INV-USSD-STATE", "INV-PHONE-ML"],
    { ttlSeconds: USSD_SESSION_TTL, phone: phoneNumber },
    async () => {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Process state machine with Latency Invariant
    const result = await invariantEngine.execute(
      ["INV-USSD-LATENCY"],
      { startTime: startTime, sessionId: sessionId },
      async () => {
        return await processUSSDState(session.state, userInput, session)
      }
    )

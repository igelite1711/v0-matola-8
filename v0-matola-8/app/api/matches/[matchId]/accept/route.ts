...
export async function POST(req: NextRequest, { params }: { params: Promise<{ matchId: string }> }) {
  ...
  try {
    ...
    await invariantEngine.execute(
      ["INV-REPUTATION-NOSHOW", "INV-MATCH-SCORE"],
      {
        noShowCount: (transporter as any).noShowCount || 0,
        score: (match as any).score || 100 // Default to pass if not scored
      },
      async () => {
        // Update match status
        return await db.updateMatch(matchId, {
          status: "accepted",

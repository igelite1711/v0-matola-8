...
/**
 * Rate limiter object for compatibility with import { rateLimiter } pattern
 * Used by app/api/disputes/route.ts
 */
export const rateLimiter = {
  checkLimit: async (identifier: string, action: string): Promise<RateLimitResult> => {
    return checkGeneralRateLimit(`${identifier}:${action}`)
  },
  checkGeneral: checkGeneralRateLimit,
  checkUSSD: checkUSSDRateLimit,
}

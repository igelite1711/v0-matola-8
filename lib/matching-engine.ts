...
function vehicleCapacityMatch(
  ...
  // First check cargo compatibility
  const compatibleVehicles = CARGO_VEHICLE_COMPATIBILITY[cargoType] || CARGO_VEHICLE_COMPATIBILITY.general
  if (!compatibleVehicles.includes(vehicleType)) {
    return {
      score: 0,
      eligible: false,
      reason: `${vehicleType} cannot transport ${cargoType} cargo`,
    }
  }

  // Check weight capacity
  const maxCapacity = VEHICLE_CAPACITIES[vehicleType]
  if (shipmentWeight > maxCapacity) {
    return {
      score: 0,
      eligible: false,
      reason: `Load (${shipmentWeight}kg) exceeds ${vehicleType} capacity (${maxCapacity}kg)`,
    }
  }

  // Calculate efficiency score (prefer right-sized vehicles)
  const utilizationRatio = shipmentWeight / maxCapacity
  let score = 0
...
export function calculateMatchScore(
  ...
  const vehicleMatch = vehicleCapacityMatch(
  ...
  return { score: Math.round(score * 100), eligible: true }
}

export function findMatches(shipment: Shipment, transporters: Transporter[], limit = 10): Match[] {
  const matches: Match[] = []
...
export function canTransporterHandleShipment(
  transporter: Transporter,
  shipment: Shipment,
...
export function getRecommendedLoads(
  transporter: Transporter,
  shipments: Shipment[],
  limit = 5,
